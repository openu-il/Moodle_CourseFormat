 ...
 *
 * $Id: section_classes.js,v 1.35.2.12 2009/12/09 01:59:47 rwijaya Exp $
 */
...
section_class.prototype.init_buttons = function() {
	...
	//openu yifatsh do not remove the maker buttom in course format ouil 11/2010
	var	countToIndex=0;
	if(main.getString('courseformat', this.sectionId) == "ouil" ){
		countToIndex=2;
	}
    for (var i=(commandContainerCount-1); i>countToIndex; i--) {
        commandContainer.removeChild(commandContainer.childNodes[i])
    }
    if (main.getString('courseformat', this.sectionId) != "weeks" &&  main.getString('courseformat', this.sectionId) != "ouil" && this.sectionId > 0) {
...
section_class.prototype.process_section = function() {
    ...
    for (var i=0;i<resource_count;i++) {
        ...
        this.resources[this.resources.length] = new resource_class(resource.id, 'resources', null, this,resource.name );
    }
...
section_class.prototype.move_to_section = function(target) {
    ...
    for (var i=loopStart; eval(loopCondition); eval(loopInc)) {
			//yifatsh openu 1/2011 swap section header
			var 	sectionHeader1="sectionheader-"+i;
			var 	sectionHeader2="sectionheader-"+eval(loopmodifier);
        ...
        if ((main.sections[i] == this) && !found) {
			...
			if(main.getString('courseformat', this.sectionId) == "ouil"){
					var txt1="";
					var txt2="";
					if( document.getElementById(sectionHeader1).innerHTML)	 txt1=document.getElementById(sectionHeader1).innerHTML;
					if (document.getElementById(sectionHeader2).innerHTML )	 txt2=document.getElementById(sectionHeader2).innerHTML;
					document.getElementById(sectionHeader1) .innerHTML=txt2;
					document.getElementById(sectionHeader2) .innerHTML=txt1;
			}
 ...
 */
function resource_class(id,group,config,parentObj, name) {
    this.init_resource(id,group,config,parentObj, name);
}
...
resource_class.prototype.init_resource = function(id, group, config, parentObj, name) {
    if (!id) {
...
resource_class.prototype.delete_button = function() {
    ...
    if (!confirm(main.getString('deletecheck', main.getString(this.is)+" \""+this.name + "\""))) {
        return false;
    }
